...
class TestKmeansClustering(unittest.TestCase):
    def setUp(self):
        ...
        self.syn_dataset = np.array(
            [
                np.concatenate(
                    (np.random.normal(100, 1, 500), np.random.normal(10, 1, 500))
                ),
                np.concatenate(
                    (np.random.normal(10, 1, 500), np.random.normal(100, 1, 500))
                ),
            ]
        ).reshape(-1, 2)
    ...
    def test_same_cluster_assignments_with_same_random_state(self):
        # Run kmeans with a specific random_state
        # and store the cluster assignments
        kmeans(self.adata, self.features, self.layer, random_state=42)
        first_run_clusters = self.adata.obs["kmeans"].copy()

        # Reset the kmeans annotation and run again with the same random_state
        del self.adata.obs["kmeans"]
        kmeans(self.adata, self.features, self.layer, random_state=42)

        # Check if the cluster assignments are the same
        self.assertTrue((first_run_clusters == self.adata.obs["kmeans"]).all())
    ...
    def test_typical_case(self):
        ...
        self.assertEqual(self.adata.uns["kmeans_features"], self.features)
    ...
    def test_invalid_n_clusters(self):
        ...
        # n_clusters argument is not a positive integer.
        with self.assertRaises(ValueError):
            kmeans(self.adata, self.features, self.layer, "invalid")

    def test_clustering_accuracy(self):
        kmeans(self.syn_data, self.features, "counts", n_clusters=2)

        self.assertIn("kmeans", self.syn_data.obs)
        self.assertEqual(len(np.unique(self.syn_data.obs["kmeans"])), 2)
